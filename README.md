...
> Task 3 已升级为 v3.4，统一使用 `src/task3.py`、共享切分表和折内预处理，按官方 ACC3 选模，并新增无权重/类别权重消融。上表中的 Task 3 指标必须在 v3.4 完整重跑后替换，不能直接作为最终论文结果。
...
| Task3 选模 | CV Accuracy 优先 | 对齐官方 ACC3；Macro-F1、Balanced Accuracy 为次级指标 |
...
| Task 3 | `results/metrics/raw/task3_tuning.csv` | 逻辑回归正则化参数 C 的训练集五折小范围搜索记录 |
...
| Task 3 | `results/metrics/raw/task3_run_complete.json` | 仅在全部结果成功保存后生成的完整运行标记 |
...
> 该公式只记录 v3.1 历史基线。最终论文必须从 `results/metrics/raw/task3_metrics.json` 或 `task3_model_comparison.csv` 读取 v3.4 的 ACC3，再重新计算总分。
...
5. 按赛题要求，任务预测结果保留 `Person_ID + True_Label + Predicted_Label`。
