"""
Task 3 - Wellness Category Prediction (v3.1 最终优化版)
- 使用 clean_utils 共享清洗函数
- 自动选择最优单模型（CV制导选择，不再盲目录入 Ensemble）
- 四分类（包含 Poor 114 条）
...
from config import (
    DATA_PATH, TASK3_DIR, SPLITS_DIR,
...
print("=" * 65)
print("Task 3: Wellness Category Prediction (v3.1)")
print("  Target: 4-class (Excellent/Good/Average/Poor)")
print("  class_weight='balanced' for handling Poor minority")
print("=" * 65)
...
EXCLUDED = list(TASK3_LEAKY_FEATURES) + [
    "Healthy_Aging_Score", "Early_Waker",
...
for i, label in enumerate(wc_le.classes_):
    print(f"    {label}: train={sum(y_train==i)}, val={sum(y_test==i)}")

# ==================== 5. CV (class_weight=balanced for Poor handling) ====================
print("\n[5/6] 5-Fold CV (with class_weight='balanced')...")
cv = StratifiedKFold(n_splits=N_FOLDS, shuffle=True, random_state=RANDOM_STATE)

models = {
    "Logistic Regression": LogisticRegression(max_iter=3000, random_state=RANDOM_STATE, C=1.0, class_weight="balanced"),
...
if hasattr(best_model, "feature_importances_"):
    ...
    imp_df.to_csv(os.path.join(TASK3_DIR, "feature_importance.csv"), index=False)

pickle.dump(best_model, open(os.path.join(TASK3_DIR, "best_model.pkl"), "wb"))

metrics = {"ACC3": float(final_acc), "best_model": best_name, "features_used": len(feat_cols),
           "n_classes": len(wc_le.classes_), "classes": list(wc_le.classes_),
           "excluded": EXCLUDED, "cv_results": cv_results,
           "note": "4-class with class_weight=balanced for Poor handling"}
...
with open(os.path.join(TASK3_DIR, "metrics.txt"), "w", encoding="utf-8") as f:
    ...
    f.write(f"Classes = {list(wc_le.classes_)} (4-class, class_weight=balanced)\n")
...
print(f"\n{'=' * 65}")
print(f"TASK 3: ACC3={final_acc:.4f}, Model={best_name}, Score={score:.2f}/40.00")
print(f"{'=' * 65}")
